"""
第三版对于加入rician衰落的CIR数据重建误差较大，该版本进行优化

新增 MeasurementScaler（测量缩放器），并提供随机投影器的稠密形式，使压缩观测可被归一化，并可复用于测量一致性正则化。
升级 ResidualBiGRU：加入专用嵌入（embeddings）、dropout 与多头注意力，在残差预测过程中更好地融合测量信息与 PCA 先验。
引入幅度/相位感知损失与测量对齐损失，并在训练与评估中接入相应的归一化资产（pipeline wiring），使其贯穿全流程生效。

另外：解决只进行上述修改的内存爆炸
FastJLProjector：新增缓存的 Torch buffers 与可微分的 project_torch 路径，并加入安全防护，避免将矩阵扩展为不可行的超大规模稠密形式。
BiGRU 训练循环：更新为可同时接受稠密张量或新的投影算子，从而在不进行稠密实体化的情况下，仍可使用测量一致性损失。
训练流水线：改为在训练过程中传递隐式投影器而非强制稠密转换，同时保持测量统计的处理方式不变。
"""
...
class MeasurementScaler:
    """对压缩观测 ``y`` 做逐维标准化，缓解不同 tap 能量差异带来的训练不稳。"""
    ...
    def fit(self, Y_list):
        ...
        self.mean = Y_all.mean(axis=0, keepdims=True).astype(np.float32)
        std = Y_all.std(axis=0, keepdims=True).astype(np.float32)
        std[std < self.eps] = 1.0
        self.std = std
        return self

    def transform(self, Y_list):
        return [((y - self.mean) / self.std).astype(np.float32) for y in Y_list]

    def inverse_transform(self, Y_list):
        return [(y * self.std + self.mean).astype(np.float32) for y in Y_list]
...
class FastJLProjector:
    ...
    def __init__(self, D, M, seed=2025):
        ...
        self.rows = rng.choice(self.D, size=self.M, replace=False).astype(np.int64)
        self._sign_tensor = torch.from_numpy(self.sign)
        self._rows_tensor = torch.from_numpy(self.rows)
    ...
    def as_dense(self):
        total_elems = self.M * self.D
        # 约束极端大规模情形，防止一次性展开触发 OOM
        if total_elems > 2e8:
            raise MemoryError(
                "FastJLProjector.as_dense() 需要展开一个形状为 "
                f"({self.M}, {self.D}) 的矩阵，元素数量约 {total_elems:.2e}。"
                "请改用 project()/project_torch() 接口以避免内存爆炸。"
            )

        from scipy.fft import dct

        eye = np.eye(self.D, dtype=np.float32)
        dct_mat = dct(eye, type=2, norm='ortho', axis=1)
        phi = dct_mat[self.rows, :]
        phi = phi * self.sign
        phi = phi * math.sqrt(self.D / self.M)
        return phi.astype(np.float32, copy=False)

    def project_torch(self, x: torch.Tensor) -> torch.Tensor:
        """在 PyTorch 图中执行与 ``project`` 等价的投影。"""

        if x.size(-1) != self.D:
            raise ValueError(f"输入特征维度应为 {self.D}，得到 {x.size(-1)}")

        orig_shape = x.shape[:-1]
        x_flat = x.reshape(-1, self.D)

        sign = self._sign_tensor.to(x.device)
        rows = self._rows_tensor.to(x.device)

        z = x_flat * sign
        Zf = self._torch_dct_type2(z)
        y = torch.index_select(Zf, dim=1, index=rows)
        y = y * math.sqrt(self.D / self.M)
        return y.reshape(*orig_shape, self.M)

    def _torch_dct_type2(self,x: torch.Tensor) -> torch.Tensor:
        """Torch 版 DCT-II (norm='ortho')，兼容自动求导。"""

        N = x.size(-1)
        if N == 0:
            return x.clone()

        v = torch.cat([x, torch.flip(x, dims=[-1])], dim=-1)
        ...
        y0 = y[..., :1] / math.sqrt(N)
        if N > 1:
            y_rest = y[..., 1:] * math.sqrt(2.0 / N)
            y = torch.cat([y0, y_rest], dim=-1)
        else:
            y = y0
        return y
...
class ResidualBiGRU(nn.Module):
    """融合测量与 PCA 粗重建的残差预测网络。"""

    def __init__(self, M, D, hidden=256, num_layers=2, dropout=0.1, attn_heads=8):
        super().__init__()
    ...
    def forward(self, y_seq, xp_seq):   # [B, T, M], [B, T, D]
        y_feat = self.y_embed(y_seq)
        xp_feat = self.xp_embed(xp_seq)
        inp = torch.cat([y_feat, xp_feat], dim=-1)
        inp = self.pre_dropout(inp)
...
# 训练循环
def _complex_mag_unit(x, eps=1e-6):
    re, im = torch.chunk(x, 2, dim=-1)
    mag_sq = re * re + im * im + eps
    mag = torch.sqrt(mag_sq)
    inv_mag = torch.rsqrt(mag_sq)
    cos = re * inv_mag
    sin = im * inv_mag
    return mag, cos, sin
...
def train_bigru(model, loader, epochs=100, lr=1e-3, clip=1.0,
                meas_matrix=None, meas_mean=None, meas_std=None,
                meas_weight=0.05, amp_weight=0.1, phase_weight=0.05):
    """训练残差预测用的 BiGRU 网络。
    ...
    amp_loss_fn = nn.SmoothL1Loss()
    phase_loss_fn = nn.SmoothL1Loss()
    meas_tensor = None
    meas_operator = None
    if meas_matrix is not None:
        if torch.is_tensor(meas_matrix):
            meas_tensor = meas_matrix.to(device)
        elif hasattr(meas_matrix, 'project_torch'):
            meas_operator = meas_matrix
        else:
            raise TypeError("meas_matrix 需为 torch.Tensor 或具备 project_torch 方法的投影器。")
    if meas_mean is not None and meas_std is not None:
        meas_mean = meas_mean.to(device).view(1, 1, -1)
        meas_std = meas_std.to(device).view(1, 1, -1)
    ...
    for ep in range(1, epochs+1):
        ...
        for y, xp, x in loader:
            ...
            x_hat = xp + r_hat
            if amp_weight > 0 or phase_weight > 0:
                mag_hat, cos_hat, sin_hat = _complex_mag_unit(x_hat)
                mag_gt, cos_gt, sin_gt = _complex_mag_unit(x)
                if amp_weight > 0:
                    amp_loss = amp_loss_fn(mag_hat, mag_gt)
                    loss = loss + amp_weight * amp_loss
                if phase_weight > 0:
                    phase_loss = phase_loss_fn(cos_hat, cos_gt) + \
                                 phase_loss_fn(sin_hat, sin_gt)
                    loss = loss + phase_weight * phase_loss
            if (meas_tensor is not None or meas_operator is not None) and meas_weight > 0:
                if meas_tensor is not None:
                    y_hat = torch.matmul(x_hat, meas_tensor.t())
                else:
                    y_hat = meas_operator.project_torch(x_hat)
                if meas_mean is not None and meas_std is not None:
                    y_hat = (y_hat - meas_mean) / meas_std
                meas_loss = loss_fn(y_hat, y)
                loss = loss + meas_weight * meas_loss
            loss.backward()
            if clip is not None:
...
def run_pipeline(
    ...
    cir_key='CIR',
    out_dir='./outputs_Racian',
    pca_components=64,
    compress_ratio=0.25,
    win_len=256, win_stride=128,   # ★ 新增
    batch_size=2, epochs=100, lr=1e-3
):
    ...
    if Nlink >= 2:
        Y_train_for_fit = [Y_list[i] for i in tr_idx]
    else:
        Y_train_for_fit = [Y_list[0][:t_split]]
    y_scaler = MeasurementScaler().fit(Y_train_for_fit)
    Y_std = y_scaler.transform(Y_list)

    if hasattr(Phi, 'project_torch'):
        meas_operator = Phi
    else:
        meas_operator = torch.from_numpy(np.asarray(Phi, dtype=np.float32))
    meas_mean = torch.from_numpy(y_scaler.mean.astype(np.float32))
    meas_std = torch.from_numpy(y_scaler.std.astype(np.float32))

    train_ds = None
    test_ds = None

    if Nlink >= 2:
        # sklearn 的 ``train_test_split`` 在样本数非常少时（例如 Nlink==2）仍然可能
        # 因随机性导致测试集为空。若出现此情况则退化为“最后 1 条链路”用作测试，
        # 以避免后续 ``DataLoader`` 访问未赋值的 ``test_ds``。
        if len(te_idx) == 0:
            te_idx = tr_idx[-1:]

        # 训练：滑窗小片段
        train_ds = SlidingWindowDataset(
            [Y_std[i] for i in tr_idx],
        ...
        if len(train_ds) == 0:
            ...
            # 测试：整段（用后 30%）。若尾段长度不足（例如序列极短导致切分为空），
            # 则回退为取序列末尾一段样本，确保测试集至少包含一个有效条目。
        y_tail = Y_std[0][t_split:]
        x_tail = X_std[0][t_split:]
        xp_tail = X_pca_std[0][t_split:]

        if y_tail.shape[0] == 0:
            total_T = Y_std[0].shape[0]
            tail_len = min(total_T, max(1, win_len))
            y_tail = Y_std[0][-tail_len:]
            x_tail = X_std[0][-tail_len:]
            xp_tail = X_pca_std[0][-tail_len:]
    ...
    # 同时返回关键对象，方便继续实验
    return {
    'Phi': Phi, 'PCA': pca, 'Scaler': scaler, 'Y_scaler': y_scaler,
...
RECON_NPY = os.path.abspath(os.path.join("outputs_Racian", "recon_cir.npy"))
OUTDIR = r"D:\PythonProject\viz_out_Racian"
LINK_IN_MAT = 0
LINK_IN_RECON = 0
