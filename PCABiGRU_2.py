"""
第一版在运行过程中内存爆了，该版把“显式 Φ”换成隐式快速随机投影（Fast JL，用 DCT 版 SRHT 思想），完全不分配 M×D，只在投影时做 sign-flip → DCT → 选行
"""
import os
import math
import random
...
#     # 高斯随机测量矩阵 Φ ∈ R^{M×D}，列归一化以增强稳定性
#     Phi = np.random.randn(M, D).astype(np.float32)
#     Phi /= np.linalg.norm(Phi, axis=0, keepdims=True) + 1e-8
#     return Phi
#
# def project_measurements(X_list, Phi):
#     # y = Φ x : X_list 每个是 [T, D]，返回 [T, M]
...
class ResidualBiGRU(nn.Module):
    """
    输入: y_seq ∈ R^{T×M}  (压缩测量序列)
    输出: r_hat ∈ R^{T×D}  (对标准化域的残差估计)
    """
    ...
    def forward(self, y_seq):   # [B, T, M]
        h, _ = self.gru(y_seq)  # [B, T, 2H]
        r = self.head(h)        # [B, T, D]
        return r

# # 数据集：每个样本是一条 link 的完整序列（也可改成滑窗小片段）
# class LinkSequenceDataset(Dataset):
#     def __init__(self, Y_list, Xstd_list, Xpca_list):
#         """
#         Y_list: 压缩测量 [T, M]
#         Xstd_list: 标准化后的真值 X_std [T, D]
#         Xpca_list: PCA 粗重建 X_pca_std [T, D]  (与 Xstd 同域)
#         """
...
#         x_std = self.Xstd_list[idx].astype(np.float32)
#         x_pca_std = self.Xpca_list[idx].astype(np.float32)
#         # 目标残差： r = x_std - x_pca_std
#         r = x_std - x_pca_std
#         return torch.from_numpy(y), torch.from_numpy(r)
...
class LinkSequenceDataset(Dataset):
    ...
    从多条 link 的序列中抽取固定长度的滑窗片段：
      ...
      返回单个样本：
        y_win [win_len, M]
        r_win [win_len, D]  其中 r = x_std - x_pca_std
    """
    def __init__(self, Y_list, Xstd_list, Xpca_list, win_len=256, stride=128, dtype=np.float32):
        assert len(Y_list) == len(Xstd_list) == len(Xpca_list)
        self.Y_list = Y_list
        self.Xstd_list = Xstd_list
        self.Xpca_list = Xpca_list
...
def run_pipeline(
    mat_path='C:/Users/admin/Desktop/gen_dd_channel/CIR.mat',
    cir_key='CIR',
    out_dir='./outputs',
    pca_components=64,
    compress_ratio=0.25,
    ...
    # ---- 全局标准化 ----
    scaler = GlobalScaler().fit(X_list)
    X_std = scaler.transform(X_list)                    # 标准化域

    # ---- 测量矩阵与压缩测量 ----
    M = max(8, int(compress_ratio * D))
    Phi = build_measurement_matrix(D, M)               # [M, D]
    Y_list = project_measurements(X_std, Phi)          # list of [T, M]

    # ---- PCA 分支（在标准化域上拟合） ----
    pca = PCABranch(n_components=min(pca_components, D//2)).fit(X_std)
    ...
    with torch.no_grad():
        for (y, r_gt) in tqdm(test_loader, desc="Evaluate"):
            # y: [1, T, M], r_gt: [1, T, D] (标准化域的残差真值)
            y = y.to(device)
            r_hat = model(y).cpu().numpy()[0]        # [T, D] in std-domain
            # 拿到对应的 PCA 粗重建（同一个 batch 的单条 link）
            # 注意：DataLoader 不带 idx，这里直接用 batch 顺序配对 test_ds
            idx = len(recon_all)
            x_pca_std = test_ds.Xpca_list[idx]       # [T, D]
            x_std_gt  = test_ds.Xstd_list[idx]       # [T, D]

            x_std_hat = x_pca_std + r_hat            # 标准化域融合
            # 反标准化回原值域
            x_hat = scaler.scaler.inverse_transform(x_std_hat)
            x_gt  = scaler.scaler.inverse_transform(x_std_gt)
...
if __name__ == "__main__":
    result = run_pipeline(
        mat_path='C:/Users/admin/Desktop/gen_dd_channel/result/CIR.mat',
        cir_key='CIR',
        out_dir='./outputs',
...
# ---- 配置：路径 & 选择的链路/Rx/Tx ----
MAT_PATH = r"C:\Users\admin\Desktop\gen_dd_channel\result\CIR.mat"
MAT_KEY = "CIR"
# RECON_NPY = r"D:\PythonProject\outputs\recon_cir.npy"  # 重建输出
RECON_NPY = os.path.abspath(os.path.join("outputs", "recon_cir.npy"))
OUTDIR = r"D:\PythonProject\viz_out2"
LINK_IN_MAT = 0
LINK_IN_RECON = 0
...
# ---- 图1：|h| 热力图（真值/重建）----
for arr, name in [(H_true, "true"), (H_hat, "recon")]:
    plt.figure(figsize=(9, 4))
    plt.imshow(20 * np.log10(np.abs(arr) + 1e-12), aspect='auto', origin='lower')
    plt.colorbar(label='|h| (dB)')
    plt.xlabel('time index');
    plt.ylabel('tap index')
    plt.title(f'|h| heatmap ({name})  Rx{IRX}/Tx{ITX}')
    plt.tight_layout()
    plt.savefig(os.path.join(OUTDIR, f"heatmap_{name}.png"))
    plt.close()

# ---- 图2：误差热力图 ----
E = np.abs(H_true - H_hat)
plt.figure(figsize=(9, 4))
plt.imshow(20 * np.log10(E + 1e-15), aspect='auto', origin='lower')
plt.colorbar(label='|h - ĥ| (dB)')
plt.xlabel('time index');
plt.ylabel('tap index')
plt.title('Error heatmap')
plt.tight_layout()
plt.savefig(os.path.join(OUTDIR, "heatmap_error.png"))
plt.close()
