...
par.ricianK_dB     = 3;                  % Rician K 因子（dB），控制直射/散射功率比例
par.ricianNumWaves = 16;                 % 通过和波法生成瑞利散射分量时的子波数量
par.ricianMinFd    = 5;                  % 散射分量的最小多普勒展开 [Hz]，保证小位移时也有起伏
...
for i_link = 1:Nlink
    ...
    switch par.modelType
        ...
        case 'virtualMotion'  % 虚拟运动：固定空间参数 + 多普勒/时间起伏
            ...
            d0_main = d0_init(ind);  % 循环前已算好的每径初始光程 d0
            ...
            % 基于和波法为每条径生成具有 Bessel 相关性的瑞利散射分量
            ricianK_lin   = 10.^(par.ricianK_dB/10);
            ricianWaves   = max(2, par.ricianNumWaves);
            losWeight     = sqrt(ricianK_lin ./ (ricianK_lin + 1));
            scatterWeight = sqrt(1 ./ (ricianK_lin + 1));
            diffuseField  = zeros(Npath, par.Ntime);
            onesTime      = ones(1, par.Ntime);
            for i_path = 1:Npath
                fd_mag = max(abs(fdop(i_path)), par.ricianMinFd);   % 确保小位移也有频散
                theta_m = (1:ricianWaves) * pi / (ricianWaves + 1);
                phi_m   = 2*pi*rand(1, ricianWaves);
                phaseGrid = 2*pi * fd_mag * cos(theta_m(:)) * tvec;
                phaseGrid = phaseGrid + phi_m(:) * onesTime;
                diffuseField(i_path, :) = sqrt(2/ricianWaves) * sum(exp(1j * phaseGrid), 1);
            end
            ...
            for i_time = 1:par.Ntime
                ...
                for i_path = 1:Npath
                    ...
                    if hasSc
                        d1    = max(norm(txc - txs), par.d_min);
                        d2_0  = max(norm(rxs - rxc0), par.d_min);
                        d2_t  = max(norm(rxs - rxc_t), par.d_min);
                        d_t   = d1 + d2_t;
                        d0_eff = d1 + d2_0;            % 与限幅一致的初始几何光程
                        amp_scale = d2_0 / d2_t;       % Rx-Scatter 段的自由空间衰落比
                    else
                        d0_eff = max(d0_init(i_path), par.d_min);
                        d_t    = max(norm(txc - rxc_t), par.d_min);
                        amp_scale = d0_eff / d_t;      % LOS：总光程的自由空间衰落比
                    end
                    % 幅度限幅
                    amp_scale = min(max(amp_scale, par.amp_floor), par.amp_clip);
                    % 复增益
                    base_amp   = db2mag(powe(i_path)) * amp_scale;
                    base_phase = phasor(i_path) * exp(-1j * 2*pi * par.fc * (d_t - d0_eff) / SoL);
                    scatter_sample = diffuseField(i_path, i_time);
                    field = base_amp * (losWeight * base_phase + scatterWeight * scatter_sample);

                    g_rx = rx.antGain(:,i_path);
                    g_tx = tx.antGain(:,i_path);

                    if ~all(isfinite([base_amp, base_phase, scatter_sample, field])) || any(~isfinite(g_rx)) || any(~isfinite(g_tx))
                        CIR(i_link,:,:,i_path,i_time) = 0;     % 兜底
                    else
                        CIR(i_link,:,:,i_path,i_time) = g_rx * field * g_tx.';
                    end
                end
            end
